...
def pill(text, color, outline=False):
    if outline:
        ui.label(text).style(
            ...
            f"padding:3px 12px;border-radius:999px;white-space:nowrap;"
        )
    else:
        ui.label(text).style(
...
def main_page():
    ...
    with ui.header().classes("items-center no-wrap").style(
        ...
        ui.label("Immigration Stamp Review").style(
            "color:white;font-weight:700;font-size:16px;margin-left:6px;")
    ...
    with ui.left_drawer(value=True, bordered=True).props("width=300").style(
        ...
        with ui.column().style("width:100%;gap:12px;padding:16px;"):
            ui.label("WORKSPACE").style(
                f"font-size:11px;font-weight:700;letter-spacing:1.2px;color:{TEXT_TER};")

            upload = ui.upload(
                label="Upload passport images", multiple=True, auto_upload=True
            ).props("accept=.jpg,.jpeg,.png flat").classes("w-full").style(
                f"border:1.5px dashed {BORDER};border-radius:{R_MD};")

            summary_slot = ui.column().classes("w-full")
            doc_list = ui.column().classes("w-full").style("gap:6px;")
            export_slot = ui.column().classes("w-full")

    main = ui.column().classes("w-full").style("gap:0;padding:24px;max-width:1400px;")
    ...
    def render_sidebar():
        ...
        with doc_list:
            for name, d in docs.items():
                ...
                with ui.card().style(
                    ...
                    with ui.row().classes("items-center justify-between w-full").style("gap:6px;flex-wrap:nowrap;"):
                        ui.label(name).style(
        ...
        with export_slot:
            if verified:
                ui.button("Export Verified Records", on_click=export_verified).props(
                    "unelevated"
                ).style(
                    f"background:{RED};color:white;border-radius:{R_MD};"
                    f"font-weight:600;width:100%;margin-top:6px;")
            any_feedback = any(
    ...
    def render_table():
        ...
        with ui.element("div").style(
            f"display:grid;grid-template-columns:{GRID};gap:10px;padding:8px 14px;"
            f"background:{BG_ALT};border-radius:{R_MD} {R_MD} 0 0;align-items:center;"
        ):
            ui.label("")
        ...
        with ui.column().style(
            ...
            for s in ordered:
                ...
                with ui.element("div").style(
                    f"display:grid;grid-template-columns:{GRID};gap:10px;padding:10px 14px;"
                    f"align-items:center;background:{row_bg};border-left:3px solid {c};"
                    ...
                    with ui.column().style("gap:0;min-width:0;"):
                        ui.label(s["id"]).style(
                            f"font-size:11px;color:{TEXT_TER};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;")
                        ui.label(f"Det. {s['det_conf']:.0%}").style(
                            f"font-size:10px;color:{TEXT_TER};white-space:nowrap;")
                    if val:
                        with ui.column().style("gap:2px;width:100%;min-width:0;"):
                            if conf is not None:
                                with ui.element("div").style(
                                    f"background:{SURFACE};border-radius:{R_PILL};height:4px;overflow:hidden;"
                                ):
                        ...
                        ui.label("No date — click to add").style(
                            f"font-size:12px;color:{RED};font-weight:600;white-space:nowrap;")
            ...
            if shown:
                for s in removed:
                    with ui.row().classes("items-center justify-between w-full").style(
                        f"padding:8px 14px;background:{BG_ALT};border-radius:{R_MD};"
                        f"margin-top:4px;opacity:.6;flex-wrap:nowrap;"
                    ):
    ...
    def render_doc_header():
        ...
        def toggle_verify():
            if is_verified:
                ...
                verified.add(name)
            render_sidebar()
            render_doc_header.refresh()

        with ui.row().classes("items-center justify-between w-full").style(
            f"border-bottom:1px solid {SURFACE};padding-bottom:12px;margin-bottom:16px;flex-wrap:wrap;gap:8px;"
        ):
            with ui.row().classes("items-center").style("gap:10px;min-width:0;"):
                ui.button(
                    icon="check_circle" if is_verified else "radio_button_unchecked",
                    on_click=toggle_verify,
                ).props("flat round dense").style(
                    f"color:{RED if is_verified else BORDER};"
                ).tooltip("Mark as reviewed" if not is_verified else "Reviewed — click to unmark")

                with ui.column().style("gap:4px;min-width:0;"):
                    ui.label(name).style(
                        f"font-size:17px;font-weight:700;color:{TEXT_PRI};line-height:1.2;"
                        f"overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:420px;")
    ...
    def render_main():
        ...
        with main:
            ...
            with ui.row().classes("w-full").style("gap:24px;align-items:flex-start;flex-wrap:wrap;"):
                with ui.column().style(f"flex:0 0 {VW}px;min-width:{VW}px;gap:4px;"):
