...
VW, VH = 520, 640         # FIXED: slightly narrower to prevent overlap, taller for documents
...
def main_page():
    ...
    # 1. FIX: Moved Workspace to a left_drawer and created a proper header
    with ui.header().classes("w-full items-center no-wrap").style(f"background:{RED};padding:10px 16px;"):
        ui.button(icon="menu", on_click=lambda: drawer.toggle()).props("flat color=white").style("margin-right:8px;")
    ...
    # ── Sidebar Drawer ───────────────────────────────────────────────────
    with ui.left_drawer(value=True).style(f"background:{BG_ALT};border-right:1px solid {SURFACE};") as drawer:
        with ui.column().style("width:100%;gap:12px;"):
            ...
            upload = ui.upload(label="Upload passport images", multiple=True, auto_upload=True).props(
                ...
                f"border:1.5px dashed {BORDER};border-radius:{R_MD};background:{BG};")
            doc_list = ui.column().classes("w-full").style("gap:6px;")
            export_slot = ui.column().classes("w-full")

    # ── Main Content Area ────────────────────────────────────────────────
    main = ui.column().classes("w-full").style("gap:0;padding:24px;")
    ...
    def render_sidebar():
            ...
            for name, d in docs.items():
                ...
                with ui.card().style(
                    ...
                    with ui.row().classes("items-center justify-between w-full").style("gap:6px;"):
                        ...
                        if is_done:
                            ui.icon("check_circle", color=RED).style("font-size:16px;") # Sleeker verified icon in sidebar
        ...
        with export_slot:
            if docs:
                ui.label(f"{len(verified)} of {len(docs)} verified").style(
                    f"font-size:12px;font-weight:600;color:{TEXT_TER};text-align:center;width:100%;margin-top:12px;"
                )
            if verified:
    ...
    def render_document_pane():
        ...
        ui.label("Click a stamp to inspect it, or use Add Stamp to mark a missed one.").style(
            f"font-size:11px;color:{TEXT_TER};text-align:center;margin-top:6px;width:{VW}px;"
        )
    ...
    def render_table():
        ...
        with ui.column().style(
            ...
            f"border-radius:0 0 {R_MD} {R_MD};max-height:600px;overflow-y:auto;"
        ):
            if not ordered:
    ...
    # 4. FIX: Decoupled header logic into a refreshable so we can toggle verification without reloading everything
    @ui.refreshable
    def render_doc_header():
        ...
        with ui.row().classes("items-center justify-between w-full").style(
            ...
            with ui.row().classes("items-center").style("gap:8px;"):
                # Sleek Icon Toggle Button for Verification
                icon_color = "green" if is_verified else "grey"
                icon_name = "check_circle" if is_verified else "radio_button_unchecked"
                ui.button(icon=icon_name, on_click=toggle_verify).props(f"flat round color={icon_color}").tooltip("Toggle Verified Status")
                
                with ui.column().style("gap:4px;"):
                    ui.label(name).style(f"font-size:18px;font-weight:700;color:{TEXT_PRI};line-height:1;")
    ...
    def render_main():
        ...
        with main:
            ...
            # REMOVED the "All documents verified" blocking logic entirely!
            
            render_doc_header()

            # 2. FIX: flex-wrap ensures components drop below instead of overlapping on small screens
            with ui.row().classes("w-full").style("gap:24px;align-items:flex-start;flex-wrap:wrap;"):
                with ui.column().style("flex:1 1 520px;min-width:0;gap:4px;"):
                    overline("Document View")
                    render_document_pane()

                with ui.column().style("flex:1 1 400px;min-width:0;gap:8px;"):
